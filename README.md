...
Agent Home can show one active goal without adding setup controls to the ambient
display. Edit `goal.json`, or create ignored `goal.local.json` for a private
override. The local override wins when it exists.

The goal tracker is built for plain numbers and useful math, not formulas the
user has to maintain. It computes:

- percent complete from `baseline`, `current`, and `target`
- expected progress for today from `start`, `due`, and `pace`
- ahead/behind amount versus the expected line
- required daily pace to still finish on time
- current observed pace and forecast finish date when enough data exists
...
For money goals, use currency formatting instead of `unit`:
...
For history, append check-ins instead of overwriting `current`:

```json
{
  "checkins": [
    { "at": "2026-06-08", "value": 3, "note": "First slice shipped." },
    { "at": "2026-06-09", "value": 4 }
  ]
}
```

Supported `pace` values:

- `steady`: straight-line progress, the usual `target * elapsed / total`
- `frontloaded`: expects more early progress for risky work
- `soft-start`: allows a slower start and expects more later

For decreasing goals, either set `target` lower than `baseline` or add
`"direction": "decrease"`.
